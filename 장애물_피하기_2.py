...
class DodgeGameApp:
    ...
    def __init__(self, root):
        ...
        # 기본 유저 데이터 로드 시도
        self.load_user_data(self.user_id)
    ...
    # 💾 데이터 저장 및 불러오기 모듈
    # ==========================================
    def get_save_filename(self, user_id):
        safe_id = "".join([c for c in user_id if c.isalnum() or c in ("-", "_")]).strip()
        if not safe_id:
            safe_id = "DefaultPlayer"
        return f"save_{safe_id}.json"

    def save_user_data(self):
        filename = self.get_save_filename(self.user_id)
        data = {
            "user_id": self.user_id,
            "total_points": self.total_points,
            "red_coins": self.red_coins,
            "purple_coins": self.purple_coins,
            "max_unlocked_level": self.max_unlocked_level,
            "current_level": self.current_level,
            "island_unlocked": self.island_unlocked,
            "has_shown_ending": self.has_shown_ending,
            "upgrades": {k: v["level"] for k, v in self.upgrades.items()},
            "island_upgrades": {k: v["level"] for k, v in self.island_upgrades.items()},
        }
        try:
    ...
    def load_user_data(self, user_id):
        filename = self.get_save_filename(user_id)
        if os.path.exists(filename):
            try:
                with open(filename, "r", encoding="utf-8") as f:
                    ...
                    self.red_coins = data.get("red_coins", 0)
                    self.purple_coins = data.get("purple_coins", 0)
                    self.max_unlocked_level = data.get("max_unlocked_level", 1)
                    self.current_level = data.get("current_level", 1)
                    self.island_unlocked = data.get("island_unlocked", False)
                    self.has_shown_ending = data.get("has_shown_ending", False)

                    for k, lvl in data.get("upgrades", {}).items():
                        if k in self.upgrades:
                            self.upgrades[k]["level"] = lvl

                    for k, lvl in data.get("island_upgrades", {}).items():
                        if k in self.island_upgrades:
                            self.island_upgrades[k]["level"] = lvl
                    return True
            except Exception as e:
                print(f"데이터 로드 실패: {e}")
                return False
        else:
            # 새 아이디 생성 시 초기화 후 즉시 파일 생성
            ...
            for k in self.upgrades:
                self.upgrades[k]["level"] = 1
            for k in self.island_upgrades:
                self.island_upgrades[k]["level"] = 0
            self.save_user_data()
            return False

    def handle_login_or_create(self):
        input_id = self.id_entry.get().strip()
        if not input_id:
            messagebox.showwarning("경고", "아이디를 입력해주세요!")
            return
        is_existing = self.load_user_data(input_id)
        if is_existing:
            messagebox.showinfo("성공", f"[{input_id}] 님의 저장 데이터를 성공적으로 불러왔습니다!")
        else:
            messagebox.showinfo("성공", f"새로운 아이디 [{input_id}] 가 생성되어 저장 파일이 등록되었습니다!")
        self.show_main_menu()
    ...
    def show_main_menu(self):
        ...
        # --- 🆔 아이디 생성 및 데이터 로드 UI 영역 ---
        user_card = tk.Frame(self.current_frame, bg="#313244", padx=15, pady=6)
        user_card.pack(pady=4)

        user_status = tk.Label(
            user_card,
            text=f"현재 플레이어: [{self.user_id}]",
            font=("Arial", 10, "bold"),
            fg="#89B4FA",
            bg="#313244",
        )
        user_status.pack(pady=(0, 4))

        input_box = tk.Frame(user_card, bg="#313244")
        input_box.pack()
        ...
        login_btn = tk.Button(
            ...
            fg="#11111B",
            cursor="hand2",
            command=self.handle_login_or_create,
        )
        login_btn.pack(side="left", padx=4)

        point_card = tk.Frame(self.current_frame, bg="#313244", padx=25, pady=8)
        point_card.pack(pady=4)
        ...
        self.level_desc_lbl.pack(pady=(4, 0))
